...
function formatdate (ts) {
  return dateFormat(ts, 'yyyy-mm-dd ddd hh:MM:ss TT');
}
router.post('/timein', async ctx => {
  ...
  if (!timein) {
    ...
    ctx.db.set('timein', ctx.state.user, { ts: formatdate(ts), late: isLate });
  } else {
...
router.post('/timeout', async ctx => {
  ...
  if (!timein) {
    ...
    ctx.db.set('history', ctx.state.user, { timein: timein.ts, timeout: formatdate(new Date()), late: timein.late }, { append: true, type: 'array' });
