...
const debug = require('debug')('router:private');
...
router.post('/timein', async ctx => {
  ...
  if (!timein) {
    ...
    debug(`tslimit ${tslimit} ts ${ts}`);
    let isLate = false;
    if (tslimit.diff(ts, 'seconds') < 1) {
      isLate = true;
    }
    ctx.db.set('timein', ctx.state.user, { ts: ts.toString(), late: isLate });
  } else {
...
router.post('/timeout', async ctx => {
  ...
  if (!timein) {
    ...
    ctx.db.set('history', ctx.state.user, { timein: timein.ts, timeout: getlocaltime().toString(), late: timein.late }, { append: true, type: 'array' });
...
router.get('/user', async ctx => {
  ...
  const emailHash = crypto.createHash('md5').update(uinfo.email.trim().toLowerCase()).digest('hex');
  const gravatarLink = `https://www.gravatar.com/${emailHash}.json`;
  ctx.jsonSucc({ userinfo: { username: ctx.state.user, admin: uinfo.admin, user_id: uinfo.id, gravatar: gravatarLink } });
});
